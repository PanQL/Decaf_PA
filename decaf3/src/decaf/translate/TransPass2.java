...
import decaf.tree.Tree.ArrayDefault;
...
import decaf.tree.Tree.ScopyClass;
...
public class TransPass2 extends Tree.Visitor {
	...
	public void visitAssign(Tree.Assign assign) {
		...
		case PARAM_VAR:		//参量
		case LOCAL_VAR:		//局部变量
			if(!assign.left.isVarIdentifier()){
	...
	public void visitIfSubStmt(IfSubStmt that) {
		...
		Label exit = Label.createLabel();
		tr.genBeqz(that.condition.val, exit);
		if (that.trueBranch != null) {
	...
	public void visitScopyClass(ScopyClass that) {
		...
		Temp t = tr.genDirectCall(c.getNewFuncLabel(),
				BaseType.INT);
	...
	public void visitForeachStmt(ForeachStmt that) {
		...
		// Label forget = Label.createLabel();
		Label exit = Label.createLabel();
		Temp esz = tr.genLoadImm4(OffsetCounter.WORD_SIZE);	//一个数组单位长度，4
