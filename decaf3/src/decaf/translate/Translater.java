...
public class Translater {
	...
	public void createFuncty(Function func) {	//create a Functy for the given function symbol
	...
	public void genCheckDivZero(Temp num) {
		Label exit = Label.createLabel();
		genBnez(num, exit);
	...
	public Temp genNewArray(Temp length) {
		...
		Temp unit = genLoadImm4(OffsetCounter.WORD_SIZE);	//unit为4对应的常量的Temp
		Temp size = genAdd(unit, genMul(unit, length));		//共length+1个字长
		genParm(size);
		Temp obj = genIntrinsicCall(Intrinsic.ALLOCATE);	//分配出来的大小为size的内存首地址
		genStore(length, obj, 0);
		Label loop = Label.createLabel();
		Label exit = Label.createLabel();
		Temp zero = genLoadImm4(0);		//代表常量0的Temp
		append(Tac.genAdd(obj, obj, size));	//obj = obj + size
		genMark(loop);
		append(Tac.genSub(size, size, unit));	//size = size - unit
		genBeqz(size, exit);	//if ( size == 0 ) branch exit
		append(Tac.genSub(obj, obj, unit));		//obj = obj - unit
		genStore(zero, obj, 0);		// *(obj + 0) = 0
		genBranch(loop);
		genMark(exit);
		return obj;
	}
	...
	public Temp genNewArrayWithMOMO(Temp e, Temp n, Class c) {
		...
		// Temp int_size = genLoadImm4(OffsetCounter.WORD_SIZE);
		Temp size = genAdd(unit, genMul(unit, n));	//可能需要根据e的大小区分
	...
	public void genScopy(Temp src, Temp dst){
		int time = src.size / OffsetCounter.WORD_SIZE - 1;
		if (time != 0) {
			for (int i = 0; i < time; i++) {
