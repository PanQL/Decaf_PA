...
ForeachStmt     :   FOREACH '(' BoundVariable IN Expr WhileJudge ')' Stmt
                    {
                        $$.stmt = new Tree.ForeachStmt($3.type, $3.ident, $5.expr, $6.expr, $8.stmt, $1.loc);
                    }
                ;
...
SimpleStmt      :   Expr Assignment
                ...
                |   VAR IDENTIFIER '=' Expr
                    {
                        Tree.VarIdent ident = new Tree.VarIdent($2.ident,$1.loc);
                        $$.stmt = new Tree.Assign(ident, $4.expr, $3.loc);
                    }
                |   /* empty */
                ;
...
Expr            :   Expr1
                ...
                |   LeftBra Expr FOR IDENTIFIER IN Expr Judge RightBra
                    {
...
Expr4           :   Expr5  ExprT4
                    {
                        if($2.expr != null){
                            $$.expr = new Tree.PlusPlus($1.expr, $2.expr, $2.loc);
                        }else{
                            $$.expr = $1.expr;
                        }
                    }
...
ExprT4          :   Oper4 Expr5 ExprT4
                    {
                        if($3.expr != null){
                            $$.expr = new Tree.PlusPlus($2.expr, $3.expr, $2.loc);
                        }else{
                            $$.expr = $2.expr;
                        }
                    }
...
Expr5           :   Expr6 ExprT5
                    {
                        if($2.elist != null && $2.elist.size() != 0){
                            Expr ex = $1.expr;
                            for(int i = 0; i < $2.elist.size(); i++){
                                ex = new Tree.ArrayRepeat(ex,$2.elist.get(i),$2.loc);
                            }
                            $$.expr = ex;
                        }else{
                            $$.expr = $1.expr;
...
Expr10          :   Expr11 ExprT10
                    {
                        if($2.elist != null && $2.elist.size() == 2){
                            if($2.counter == Tree.DEFAULT){
                                $$.expr = new Tree.ArrayDefault($1.expr, $2.elist.get(0), $2.elist.get(1), $1.loc);
                            }else{
                                $$.expr = new Tree.ArrayRange($1.expr, $2.elist.get(0), $2.elist.get(1), $1.loc);
                            }
                        }else{
                            $$.expr = $1.expr;
                            $$.loc = $1.loc;
...
ExprT10          :   '[' Expr Range ']' ExprT10
                    {
                        if($3.expr == null){
                            if($5.counter == Tree.DEFAULT){
...
IfBranchList	:	IfSubStmt IfBranches
					{
                        $$.stmt = new Tree.GuardedStmt($2.elist,$1.expr,$1.loc);
					}
				|	/* empty */
					{
                        $$.stmt = new Tree.GuardedStmt(new ArrayList<Expr>(), null, $$.loc);
					}
                ;

IfBranches      :   GUARD IfSubStmt IfBranches
                    {
                        $$.elist = new ArrayList<Expr>();
                        $$.elist.add($2.expr);
                        $$.elist.addAll($3.elist);
...
IfSubStmt		:	Expr1 ':' Stmt
					{
						$$.expr = new Tree.IfSubStmt($1.expr,$3.stmt,$1.loc);
					}
                ;

Constants       :   Constant ArrConstList
                    {
                        $$.elist = new ArrayList<Expr>();
                        $$.elist.add($1.expr);
                        $$.elist.addAll($2.elist);
...
ArrConstList    :   ',' Constant ArrConstList
                    {
                        $$.elist = new ArrayList<Expr>();
                        $$.elist.add($2.expr);
                        $$.elist.addAll($3.elist);
